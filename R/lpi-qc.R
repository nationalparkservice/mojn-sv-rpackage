...
#' List occurrences of duplicate plant species in LPI data
#'
#' @param conn Database connection generated from call to \code{OpenDatabaseConnection()}. Ignored if \code{data.source} is \code{"local"}.
#' @param path.to.data The directory containing the csv data exports generated from \code{SaveDataToCsv()}. Ignored if \code{data.source} is \code{"database"}.
#' @param park Optional. Four-letter park code to filter on, e.g. "MOJA".
...
#' \dontrun{
#'     conn <- OpenDatabaseConnection()
#'     LpiQcDuplicateSpecies(conn, park = "LAKE")
#'     LpiQcDuplicateSpecies(conn, spring = "LAKE_P_BLUE0", field.season = "2019")
#'     LpiQcDuplicateSpecies(path.to.data = "path/to/data", data.source = "local")
#'     CloseDatabaseConnection(conn)
#' }
LpiQcDuplicateSpecies <- function(conn, path.to.data, park, spring, field.season, data.source = "database") {
